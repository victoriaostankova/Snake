...
def change_direction(world, event):
    if event.keysym in movement_map:
        if world.current_vector == (1, 0) and movement_map[event.keysym] != (-1, 0):
            world.current_vector = movement_map[event.keysym]
        elif world.current_vector == (0, 1) and movement_map[event.keysym] != (0, -1):
            world.current_vector = movement_map[event.keysym]
        elif world.current_vector == (-1, 0) and movement_map[event.keysym] != (1, 0):
            world.current_vector = movement_map[event.keysym]
        elif world.current_vector == (0, -1) and movement_map[event.keysym] != (0, 1):
            world.current_vector = movement_map[event.keysym]
...
def main_loop(world, root):
    ...
    if os.path.exists(filePath) and os.stat(filePath).st_size != 0:        
        ...
        try:
            ...
            world.canvas.create_text(WIDTH / 2, HEIGHT / 2, text="Файл {0} поврежден! Удалите файл и перезапустите игру.".format(filePath), font="Arial 14", fill="black")
        f.close()
    else:
        j = 0    
    ...
    if result:
        ...
        root.title("Змейка. Текущий счет: {1}.".format(j, i))
    else:       
        if i > j:
            fNewRecord = open(filePath, 'w')
            fNewRecord.write(str(i))
            j = i
        world.canvas.create_text(WIDTH / 2, HEIGHT / 2, text="Конец игры!\nВаш счет: {0}\nМаксимальный рекорд: {1}".format(i, j), font="Arial 36", fill="black")
